...
describe('Pagination test', () => {
    test('renders Pagination', () => {
        const underTest = render(<Pagination pagination={{current_page:2, total_pages:10, total_count:200}} onPageClicked={jest.fn}/>)
        expect(underTest).toMatchSnapshot();
    });
    test('Only one Page', async () => {
        const pagination = {current_page:1, total_pages:1, total_count:20}
        const underTest = render(<Pagination pagination={pagination} onPageClicked={jest.fn()}/>)
    ...
    describe('Clicks', () => {
        ...
        test('Prev Button', async () => {
            const underTest = render(<Pagination pagination={pagination} onPageClicked={onClick}/>)
